...
uint8_t aes_expanded_key[Nb][AES_WORDS] = { { 0x00 } };

uint8_t round_constants_arr[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 }; // Further constants are: 0x6C, 0xD8, 0xAB, 0x4D, 0x9A, 0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A, 0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39
...
{
    uint8_t a[Nb] = { 0, 0, 0, 0 };
    MIX_COLUMN(a, state, 0, 4, 8, 12);
    MIX_COLUMN(a, state, 1, 5, 9, 13);
    MIX_COLUMN(a, state, 2, 6, 10, 14);
    MIX_COLUMN(a, state, 3, 7, 11, 15);
}

void aes_inverse_mix_columns(uint8_t state[AES_BLK_LEN])
{
    uint8_t a[Nb] = { 0, 0, 0, 0 };
    INVERSE_MIX_COLUMN(a, state, 0, 4, 8, 12);
    INVERSE_MIX_COLUMN(a, state, 1, 5, 9, 13);
    INVERSE_MIX_COLUMN(a, state, 2, 6, 10, 14);
    INVERSE_MIX_COLUMN(a, state, 3, 7, 11, 15);
}
#else
void aes_mix_columns(uint8_t state[AES_BLK_LEN])
{
    uint8_t a[Nb] = { 0, 0, 0, 0 };
    MIX_COLUMN(a, state, 0, 1, 2, 3);
    MIX_COLUMN(a, state, 4, 5, 6, 7);
    MIX_COLUMN(a, state, 8, 9, 10, 11);
    MIX_COLUMN(a, state, 12, 13, 14, 15);
}

void aes_inverse_mix_columns(uint8_t state[AES_BLK_LEN])
{
    uint8_t a[Nb] = { 0, 0, 0, 0 };
    INVERSE_MIX_COLUMN(a, state, 0, 1, 2, 3);
    INVERSE_MIX_COLUMN(a, state, 4, 5, 6, 7);
    INVERSE_MIX_COLUMN(a, state, 8, 9, 10, 11);
    INVERSE_MIX_COLUMN(a, state, 12, 13, 14, 15);
}
#endif
...
void aes_add_round_key(uint8_t *state, uint8_t round)
{
    ...
    for (uint8_t i = 0; i < Nb; i++)
        for (uint8_t j = Nb * round; j < (Nb * round) + Nb; j++)
            state[n++] = aes_expanded_key[i][j] ^ state[n];
}

void aes_sub_bytes(uint8_t *state)
{
    for (uint8_t n = 0; n < AES_BLK_LEN; n++)
...
{
    aes_encrypt_initial_round(block);

    for (uint8_t round = 1; round < Nr; round++)
        aes_encrypt_round(block, round);

    aes_encrypt_final_round(block);
}

void aes_decrypt_block(uint8_t *block)
{
    aes_decrypt_initial_round(block);

    for (uint8_t round = (Nr - 1); round > 0; round--)
        aes_decrypt_round(block, round);
